...
const index = async (req, res) => {
  ...
  return res.json({
    cls: cls,
    user: { isMainTeacher: user.isMainTeacher, userType: user.userType },
  });
};
...
const classes = async (req, res) => {
  ...
  if (activeRoutine) {
    activeClasses = await db("classes")
      ...
      .where("classes.routineId", "=", activeRoutine.id);
    ...
    cls = await db("classes")
      ...
      .join("courses", "courses.id", "=", "classes.courseId")
      .where("cp.userId", "=", user.id)
      .whereNot("classes.routineId", "=", activeRoutine.id);
  } else {
    cls = await db("classes")
      .select("classes.id", "courses.name as courseName", "courses.courseCode")
      .join("class_participants as cp", "cp.classId", "=", "classes.id")
...
const postAttendance = async (req, res) => {
  ...
  if (!error) {
    ...
    if (prevAtt) {
      return res.json({
        status: "failed",
        message: "Attendance Already Added for this date",
      });
    }
  ...
  if (!error) {
    ...
    try {
      if (id) {
        ...
        await trx("assignment_students").insert(assignData);
...
const getAssignments = async (req, res) => {
  ...
  const assignments = await db("assignments").where({ classId: classId });
...
const deleteAssignment = async (req, res) => {
  ...
  try {
    await trx("assignments").where("id", "=", a).delete();
...
const submitClassWork = async (req, res) => {
  ...
  if (user.userType !== "student") {
    return res.json({
      status: "failed",
      message: "Only Students can submit classswork",
    });
  }
  ...
  try {
    ...
    await trx("attachments").insert(attachableFiles);
  ...
  try {
    let submissions = await db("assignment_students as as")
      .select(
        ...
        "as.status",
        "as.points",
        "as.id as submissionId",
        "assignments.points as totalPoints"
      )
      .join("assignments", "assignments.id", "=", "as.assignmentId")
...
const viewSubmission = async (req, res) => {
  ...
  const submission = await db("assignment_students as as")
    .select(
      "as.id",
      "users.firstName",
      "users.lastName",
    ...
    .join("assignments", "assignments.id", "=", "as.assignmentId")
    .join("users", "users.id", "=", "as.assignedTo")
    .join("student_details", "student_details.userId", "=", "users.id")
    .where("as.id", "=", sub_id)
    .first();

  const attachments = await db("attachments")
    .where("attachable", "student_work")
    .where("attachableId", "=", submission.assignmentId)
    .where("ownerId", "=", submission.assignedTo);

  return res.json({ submission, attachments });
};
...
const addOrUpdatePoints = async (req, res) => {
  ...
  try {
    await db("assignment_students as as")
      .update({ points })
      .where("as.id", "=", sub_id);

    return res.json({ status: "success", message: "Success full" });
  } catch (er) {
